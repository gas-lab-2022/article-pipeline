...
- `outputPath`: fact-check.md の書き出し先パス
...
claims が 0 件の場合は、空の fact-check.md を `{outputPath}` に書き出して終了してください。

空の fact-check.md フォーマット：

```markdown
# ファクトチェック

- **チェック日時**: {ISO 8601形式の現在日時}
- **記事タイトル**: {記事タイトル}
- **総合判定**: pass
- **検証結果**: 0/0 verified
...
`{outputPath}` に以下の Markdown を Write ツールで書き出してください：
...
- **記事タイトル**: {記事タイトル}
- **総合判定**: {overallVerdict}
- **検証結果**: {verifiedCount}/{totalCount} verified
...
### 1. {verdict の絵文字} {claim}

- **カテゴリ**: {category}
- **該当箇所**: {source}
- **根拠**: {evidence}
- **修正提案**: {suggestion}（disputed/outdated のみ。それ以外は省略）

### 2. ...
```

verdict の絵文字は コンソール表示 セクションと同じ対応を使用してください。
