...
```bash
npx tsx scripts/wp-publish-draft.ts "articles/{slug}/article.json"
```

出力から `Post ID` を取得し、以下を実行してください：

1. `articles/{slug}/article.json` に `"wpPostId": {postId}` フィールドを追加（Edit ツール使用）
...
cp ~/path/to/screenshot.png articles/{slug}/screenshots/{filename}.png
git add articles/{slug}/screenshots/
