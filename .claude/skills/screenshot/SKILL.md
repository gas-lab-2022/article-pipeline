...
### 重要: Claude Code ターミナル画面のルール

Claude Code のターミナル画面を記事に掲載する場合は、**必ず実際の操作画面を撮影**してください（Mode A: Web 撮影でローカルターミナルを映すか、実機スクリーンショットを使用）。Mode B のターミナルモックを Claude Code の画面として使用してはいけません。読者に見せるのは本物の動作画面であるべきです。

Mode B（ターミナルモック）は、Claude Code 以外の汎用的なコマンドライン操作の説明に使用してください。
