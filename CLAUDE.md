...
WordPress ブログ「programming-zero.net」の記事生成パイプライン。Claude Code のスキル（`/generate`）で SEO 最適化された日本語記事を 8 ステップで生成し、WordPress に下書き投稿する。
...
```
.claude/skills/generate/SKILL.md    ← 記事生成パイプライン定義（8ステップ）
.claude/skills/revise/SKILL.md      ← 既存記事リライトパイプライン定義（7ステップ）
